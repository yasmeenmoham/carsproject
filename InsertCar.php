...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        $sql = "INSERT INTO `insert_cars`(`Car_title`, `content`, `price`, `model`, `Auto`, `properties`,`images`) VALUES (?,?,?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$title, $content, $price, $model, $auto,$properties,$image_name]);
		...
		<div class="container">
			<form method="POST" action="" class="m-auto" style="max-width:600px" enctype="multipart/form-data">
				...
				<div class="form-group mb-3 row"><label for="select-option1" class="col-md-5 col-form-label">Auto / Manual</label>
					<div class="col-md-7"><select class="form-select custom-select custom-select-lg" id="select-option1" name="AutoManual">
                 
							<option value=0 name="auto">Auto</option>
							<option value=1 name="Manual">Manual</option>
